...
//functoins for creating comments
...
//functoins for unfollow the user
...
function filterFollowSuggestion(){
    ...
    foreach($list as $user){
        if(!checkFollowStatus($user['id']) && count($filter_list)<11){
            $filter_list[] = $user;
        }
    }
    return $filter_list;
...
//for checking the user is followed buy current user or not
...
function createUser($data){
    ...
    $query .= " VALUES('{$first_name}','{$last_name}', '{$email}','{$username}','{$password}','{$role}')";
    return mysqli_query($db, $query);
}



//functions for verify email
...
function validateUpdateForm($form_data, $image_data){
    ...
    $response['status']=true;
        ...
        if($image_data['name']){
         ...
         if($size>1000){
            $response['msg'] = "upload image less than 2.5 mb";
            $response['status']=false;
            $response['field']='profile_pic';
         }
        }
